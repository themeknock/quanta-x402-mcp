...
def _rig(canned_page, no_tls):
    """Paid tool calls hit a canned page, not the live internet."""
    yield


def test_mcp_endpoint_lists_three_tools(client):
    names = sorted(t["name"] for t in _rpc(client, "tools/list")["result"]["tools"])
    assert names == ["check_headers", "check_tls", "check_url"]


def test_tool_descriptions_admit_the_javascript_limit(client):
    tools = {t["name"]: t["description"] for t in _rpc(client, "tools/list")["result"]["tools"]}
    assert "raw_html_only" in tools["check_url"]


def test_bogus_payment_is_refused(client, reset_facilitator):
    body = _tool(client, "check_url", {"url": URL, "payment": "bogus"})
...
def test_presence_alone_buys_nothing_on_any_tool(client, reset_facilitator):
    for name, args in [("check_url", {"url": URL}), ("check_tls", {"host": HOST}),
                       ("check_headers", {"url": URL})]:
...
def test_no_payment_returns_a_challenge_built_by_the_sdk(client, reset_facilitator):
    body = _tool(client, "check_url", {"url": URL})
...
def test_a_payment_the_facilitator_rejects_is_refused(client, reset_facilitator):
    ...
    body = _tool(client, "check_url", {"url": URL, "payment": header})
...
def test_a_verified_payment_runs_the_tool_and_settles(client, reset_facilitator):
    ...
    row = next(r for r in recent if r["surface"] == "mcp" and r["route"] == "check_url")
...
def test_settlement_failure_does_not_pretend_it_worked(client, reset_facilitator):
    ...
    header = payment_header()
    body = _tool(client, "check_headers", {"url": URL, "payment": header})
    ...
    row = next(r for r in client.get("/internal/usage?limit=5").json()["recent"]
               if r["route"] == "check_headers")
    assert row["paid"] is False
    assert row["tx_ref"] == ""


@pytest.mark.parametrize("tool,args", [("check_url", {"url": URL}),
                                       ("check_tls", {"host": HOST}),
                                       ("check_headers", {"url": URL})])
def test_every_tool_is_gated(client, reset_facilitator, tool, args):
